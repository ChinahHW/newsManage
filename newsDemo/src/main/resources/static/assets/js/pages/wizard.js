$(document).ready(function() {
	...
	function createErrorMsg (loc, msg) {
		loc.append(
			'<div class="alert alert-fail"  style="color:red"><button type="button" class="close" data-dismiss="alert">&times;</button><strong style="color:red">!!! Error!</strong> '+ msg + ' </div>'
		);
	}
 	...
 	$("#wizard").formwizard({ 
	 	...
	 	formOptions :{
			success: function(data){
				...
				if(data.code == '200'){
					createSuccessMsg($("#wizard .msg"), "用户已成功添加");
				}else{
					createErrorMsg($("#wizard .msg"), data.msg)
				}
